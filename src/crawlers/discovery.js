...
export async function fetchTextSource(url, timeout, label, proxyAgent) {
  try {
    const options = {
      method: 'GET',
      timeout: { request: Math.max(timeout || 15000, 15000) }, // minimal 15 detik
      throwHttpErrors: false,
      retry: { 
        limit: 3, // naik dari 2 ke 3
        methods: ['GET'], 
...
export async function crawlHTMLPages(base, timeout, proxyConfiguration) {
  ...
  const crawler = new CheerioCrawler({
    ...
    requestHandlerTimeoutSecs: Math.ceil((timeout || 15000) / 1000) + 10, // lebih longgar
    proxyConfiguration,
